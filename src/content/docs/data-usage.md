...
默认情况下，使用 Bedrock、Vertex 或 Foundry 时会禁用错误报告、遥测和错误报告。会话质量调查是例外，无论提供商如何，都会出现。您可以通过设置 `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` 立即选择退出所有非必要流量，包括调查。以下是完整的默认行为：|服务 | Claude API |顶点 API |基岩 API |铸造API |
| ------------------------------------------------ | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **Statsig（指标）** |默认开启。
