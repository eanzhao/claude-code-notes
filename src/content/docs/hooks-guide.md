...
* `"type": "prompt"`：单轮 LLM 评估。请参阅[基于提示的挂钩](#prompt-based-hooks)。
...
对于需要判断而不是确定性规则的决策，请使用 `type: "prompt"` 挂钩。 Claude Code 不运行 shell 命令，而是将提示和挂钩的输入数据发送到 Claude 模型（默认为 Haiku）来做出决定。如果您需要更多功能，可以使用 `model` 字段指定不同的模型。
