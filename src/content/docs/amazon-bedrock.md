...
3. 选择**聊天/文本游乐场**
4. 选择任意 Anthropic 模型，系统将提示您填写用例表
...
这些变量使用跨区域推理配置文件 ID（带有 `us.` 前缀）。如果您使用不同的区域前缀或应用程序推理配置文件，请进行相应调整。有关当前和旧版模型 ID，请参阅[模型概述](https://platform.claude.com/docs/en/about-claude/models/overview)。有关环境变量的完整列表，请参阅[模型配置](./model-config#pin-models-for-third-party-deployments)。
...
当用户在 `/model` 中选择这些版本之一时，Claude Code 将使用映射的 ARN 调用 Bedrock。没有覆盖的版本会回退到内置 Bedrock 模型 ID 或启动时发现的任何匹配的推理配置文件。有关覆盖如何与 `availableModels` 和其他模型设置交互的详细信息，请参阅[覆盖每个版本的模型 ID](./model-config#override-model-ids-per-version)。
